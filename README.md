...
- `electron/new_xml.js` — 开屏引导页的「新建 translations.xml」：选个位置即生成一份带 中文/英语 两列和示例条目的全新 XML；若上级目录没有 `gen_multilang.py` 标记会在同目录补一个（纯标记，不执行），随后立即生成配套的 `multilang.h/.c` 并进入编辑。不覆盖已有文件。
- `electron/c_sync.js` — 反向同步 multilang.c → translations.xml：一致性横幅发现 `.c` 与 XML 有出入时提供「从 multilang.c 同步到 XML」按钮（常见于同事直接改了 `.c` 没同步 XML）。解析磁盘上 `multilang.h` 的 `LANG_XX` 宏确定列序、解析 `.c` 的字符串表，更新不同的格、补充缺失的 key；只增改不删除，"值 == key" 的格按生成器空翻译兜底跳过。同步后自动重新生成，两边对齐。
...
- `src/` — 纯 JS/HTML/CSS 前端，无构建步骤，直接照搬原表格渲染/列宽拖拽/过滤/弹窗逻辑；仅把 `window.__TAURI__.core.invoke` 换成 `window.native.invoke`，关窗保护改为经 `setDirty` 同步脏状态给主进程。保存模型不变：单元格失焦不自动保存，只有点"保存" / `Ctrl+S` / `Ctrl+Enter` 才写入全部脏单元格并触发 multilang 重新生成。表格做了行虚拟化：只渲染视口附近的行，几千词条也能流畅滚动；编辑中的脏格状态记在数据模型（`EDITS`）里，行滚出屏幕被回收后再滚回来照样保留。
